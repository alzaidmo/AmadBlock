...
if nomFich != "":
    ...
    # Boucle temps que l'on est connecte
    ############################################
    while (socket.connect):
        ...
        if recu == "GO": # Si le serveur accepte on envoit le fichier
            ...
            if octets > 1024:	# Si le fichier est plus lourd que 1024 on l'envoit par paquet
            ...
            else: # Sinon on envoit tout d'un coup
                donnees = fich.read()
                socket.send(donnees)
...
##################################################################
    ...
    while (conn.connect):
        ...
        if accepte == "non": # Condition si on a pas deja envoye le nom et la taille du fichier
